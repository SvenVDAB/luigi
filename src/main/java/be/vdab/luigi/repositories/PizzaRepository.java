...
public class PizzaRepository {
    ...
    public void update(Pizza pizza) {
        var sql = """
                update pizzas set naam = ?, prijs = ?, pikant = ? where id = ?
                            """;
        if (template.update(sql, pizza.getNaam(), pizza.getPrijs(),
    ...
    public long create(Pizza pizza) {
        return insert.executeAndReturnKey(Map.of("naam", pizza.getNaam(), "prijs", pizza.getPrijs(),
    ...
    public List<Pizza> findByPrijsBetween(BigDecimal van, BigDecimal tot) {
        var sql = """
                select id, naam, prijs, pikant
                from pizzas where prijs between ? and ?
                order by prijs
                """;
